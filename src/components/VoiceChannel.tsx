...
export default function VoiceChannel({ groupId, profile, isAdmin, groupCreatorId }: VoiceChannelProps) {
  ...
  useEffect(() => {
    ...
    socketRef.current.on('user-joined-voice', async ({ socketId, profile: userProfile }) => {
      ...
      remoteUsersRef.current[socketId] = newRemoteUser;
    ...
    socketRef.current.on('current-voice-users', (users: { socketId: string, profile: Profile }[]) => {
